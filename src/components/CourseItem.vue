...
<template>
  <div class="task-item bg-gray-800 text-white p-4 rounded-lg shadow-lg flex items-center space-x-4 my-5">
    <div class="flex flex-1">
      <div>
        <h2 class="text-xl font-semibold mr-3 text-left">{{ props.course.getTitle() }}</h2>
        <p class="text-sm text-gray-400 text-left">{{ props.course.getCode() }}</p>
      </div>
    </div>
    <div class="flex">
      <button @click="modalState.openModal('editCourse', course)" class="mr-4">
      ...
      <button @click="courseManager.remove(course)">
